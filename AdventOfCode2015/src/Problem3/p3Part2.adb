...
procedure Main is
   ...
   A              : Unbounded_String;
   CurrNS, CurrEW : Integer := 0;
   RoboNS, RoboEW : Integer := 0;
   Location       : Tuple;
   D              : Character;
      ...
      if Direc mod 2 = 1 then
         ...
         exception
            when Constraint_Error =>
               Put_Line("Santa Has already gone to " & Coord_Tuple.GetElemsString(Location) &".");
         end;
         CurrNS := Location.Elem1;
         CurrEW := Location.Elem2;
      elsif Direc mod 2 = 0 then
         -- Robo Santa
         Location := Move(CurrNorth => RoboNS, CurrEast => RoboEW, Direction => D);
         Houses.Append(To_Unbounded_String(Coord_Tuple.GetElemsString(Location)));begin
         VerifyHouses.Insert(To_Unbounded_String(Coord_Tuple.GetElemsString(Location)));
         exception
            when Constraint_Error =>
               Put_Line("RobotSanta has Already gone to " & Coord_Tuple.GetElemsString(Location) &".");
         end;
         RoboNS := Location.Elem1;
         RoboEW := Location.Elem2;
      else
         Put_Line("Something broke with " & Integer'Image(Direc) & " which is the value of Direc");
      end if;
   end loop;
